...
if [ -n "${TERMUX_VERSION:-}" ] || printf '%s' "${PREFIX:-}" | grep -q 'com.termux'; then
  echo "[1/7] 운영체제: Android + Termux 확인"
else
...
if ! command -v python >/dev/null 2>&1; then
  echo "[2/7] Python 없음 → Termux Python을 설치합니다."
...
python - <<'PY'
import sys
print(f"[2/7] Python: {sys.version.split()[0]}")
...
# 7) Build initial code-only map and enable automatic refresh hooks.
echo "[6/7] 최초 코드 지도 생성"
bash ./GRAPHIFY_UPDATE.sh || exit 1

echo "[7/7] Git 변경 시 지도 자동갱신 훅 설치"
