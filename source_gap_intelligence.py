...
This module is deliberately *not* a trust learner and not a code rewriter.
It learns which already-approved discovery families are useful for a
Pokémon / ONE PIECE / NARUTO × KR / JP / US × topic cell, then prioritizes
future attempts inside a fixed source-family allowlist.

Safety invariants
- learned state cannot add hosts, source families, code, headers or credentials;
- X/Instagram/Google/search/wiki/community hits stay candidates until existing
  official verification rules confirm them;
- 401/403 are access-control blocks and are never bypassed;
- 429 obeys Retry-After/cooldown and is not retried in the same run;
- secondary targets are never fetched here: only public search result metadata
  is retained, so a community/wiki page cannot silently become canonical data;
- missing official evidence is reported as a gap, never manufactured.
"""
...
EXPECTED_CELLS = tuple((game, region, topic) for game in GAMES for region in REGIONS for topic in TOPICS)

# Fixed family IDs are the only values that learned state may prioritize.
# Trust is intentionally not encoded in the utility score.
SOURCE_FAMILIES = (
    "official_direct",
    "official_social",
    "google",
...
SOCIAL_HOSTS = {
    ...
    "youtube.com": "social_youtube", "www.youtube.com": "social_youtube", "youtu.be": "social_youtube",
}
GOOGLE_HOSTS = {"news.google.com", "www.googleapis.com", "google.com", "www.google.com"}
PRESS_HOST_HINTS = (
    "news", "press", "prtimes", "businesswire", "prnewswire", "yna.co.kr", "newsis.com",
    "famitsu.com", "dengekionline.com", "hankyung.com", "daum.net",
)
...
TOPIC_QUERY = {
    "KR": {
        ...
        "release": "출시 발매 신제품", "reprint": "재발매 재판", "merch": "굿즈 공식샵", "anniversary": "기념 주년",
    },
    "JP": {
        "event": "イベント", "tournament": "大会 チャンピオンシップ", "popup": "ポップアップ",
        "promo": "プロモ 配布", "collab": "コラボ", "movie": "映画 劇場版",
        "release": "発売 新商品", "reprint": "再販 再版", "merch": "グッズ 公式ショップ", "anniversary": "記念 周年",
    },
    "US": {
        "event": "event", "tournament": "tournament championship", "popup": "pop-up",
        "promo": "promo giveaway", "collab": "collaboration", "movie": "movie film",
        "release": "release new set", "reprint": "reprint restock", "merch": "merch official shop", "anniversary": "anniversary",
    },
}
...
def _int(value, default=0, maximum=1_000_000) -> int:
    try:
...
def _topic(row: dict) -> str:
    ...
    aliases = {"collaboration": "collab", "stock": "release"}
    explicit = aliases.get(explicit, explicit)
    if explicit in TOPICS:
        return explicit
    text = " ".join(str(row.get(k) or "") for k in (
        "category", "title", "name_ko", "name_native", "excerpt", "status", "reward", "condition"
    ))
    checks = (
...
def _game(value: object) -> str:
    ...
    if "포켓몬" in text or "pokemon" in text or "pokémon" in text:
        return "포켓몬 카드"
    if "원피스" in text or "one piece" in text or "ワンピ" in text:
        return "원피스 카드"
    if "나루토" in text or "naruto" in text or "ナルト" in text:
        return "나루토 카드"
    return str(value or "").strip() if str(value or "").strip() in GAMES else ""
...
def classify_source(row: dict) -> str:
    """Map a row to a fixed utility family; never learn a new family/host."""
    ...
    if row.get("official_domain_match") is True or str(row.get("source_grade") or "").lower() == "official" or kind in {"official_direct", "official_sitemap"}:
    ...
    if host in GOOGLE_HOSTS or provider in {"google_news", "google_cse", "google"}:
        return "google"
    if provider in {"duckduckgo", "bing", "bing_rss", "bing_news", "naver_news", "multi_provider"} or "search" in kind:
        return "broad_search"
...
def _verified(row: dict) -> bool:
    """Only explicit verification backed by existing primary trust may close a gap."""
...
def _rows_from_pipeline(social: dict | None, supplementary: dict | None, broad_blocks: list[dict] | None) -> list[dict]:
    rows: list[dict] = []
...
def audit_pipeline(social: dict | None, supplementary: dict | None, broad_blocks: list[dict] | None) -> dict:
    """Pure coverage audit. Empty cells alone do not assert that an event exists."""
    rows = _rows_from_pipeline(social, supplementary, broad_blocks)
    buckets: dict[tuple[str, str, str], dict] = {}
    for cell in EXPECTED_CELLS:
        buckets[cell] = {"candidate_count": 0, "verified_count": 0, "corroborated_count": 0, "families": {}}
    for row in rows:
    ...
    for game, region, topic in EXPECTED_CELLS:
        ...
        reasons = []
        if lead_gap:
            risk += 5; reasons.append("후보는 있으나 공식 검증근거 없음")
        if corroborated_gap:
            risk += 3; reasons.append("복수 발견근거는 있으나 공식확정 없음")
        if raw["candidate_count"] > 0 and family_count < 2:
            risk += 2; reasons.append("독립 출처군 다양성 부족")
        cell_row = {
            "game": game, "region": region, "topic": topic,
        ...
        if risk:
            priority.append(cell_row)
    priority.sort(key=lambda x: (x["risk_score"], x["corroborated_count"], x["candidate_count"]), reverse=True)
    return {
...
def _load_memory(path: Path) -> dict:
    ...
    for candidate in (path, backup):
        try:
            ...
            if isinstance(data, dict) and isinstance(data.get("cells"), dict) and isinstance(data.get("families"), dict):
                clean = _fresh_memory()
                clean["runs"] = _int(data.get("runs"))
                clean["updated_at"] = data.get("updated_at")
                for key, value in list(data.get("cells", {}).items())[:MAX_MEMORY_CELLS]:
                    if isinstance(value, dict):
                        clean["cells"][str(key)[:100]] = dict(value)
                for key, value in list(data.get("families", {}).items())[:MAX_FAMILY_STATS]:
...
def _family_score(stat: dict) -> float:
    ...
    # Utility only. It never changes trust or allowlists.
    value = (verified / discovered) * 2.2 + (corroborated / discovered) * 1.1 + 0.35 / math.sqrt(discovered)
    value -= min(0.8, blocked * 0.04 + limited * 0.02)
    return round(max(-1.0, min(4.0, value)), 5)


def preferred_families(memory: dict) -> list[str]:
    """Primary verification remains first; only discovery order is learned."""
    stats = memory.get("families") if isinstance(memory, dict) else {}
    ranked = []
    for family in DISCOVERY_FAMILIES:
        stat = stats.get(family, {}) if isinstance(stats, dict) else {}
        ranked.append((_family_score(stat), family))
    ranked.sort(key=lambda x: (x[0], -DISCOVERY_FAMILIES.index(x[1])), reverse=True)
...
def _parse_channel_failures(social: dict | None) -> dict[str, dict]:
    ...
    out = {}
    for name, row in list(status.items())[:40] if isinstance(status, dict) else []:
        if not isinstance(row, dict):
            continue
        text = " ".join(str(row.get(k) or "") for k in ("error", "status", "message", "errors"))
        lower = text.lower()
        access = bool(re.search(r"(?:status|http|httperror)\s*[: ]?\s*(?:401|403)\b", lower))
        limited = bool(re.search(r"(?:status|http|httperror)\s*[: ]?\s*429\b|retry-after", lower))
        family = {
            "x": "social_x", "instagram": "social_instagram", "google_news": "google",
            "google_cse": "google", "public_social_search": "broad_search", "route_diversity": "broad_search",
        }.get(str(name), "broad_search")
        out[str(name)[:60]] = {
            "family": family,
            "access_control_blocked": access,
            "rate_limited": limited,
            "next_action": "공개 접근권한/공식 API 상태 확인 · 자동 우회 금지" if access else ("Retry-After/cooldown 후 재확인" if limited else "정상/기타"),
        }
    return out
...
def observe_pipeline(
    ...
    *,
    ...
    per_family_run = {family: {"discovered": 0, "verified": 0, "corroborated": 0} for family in SOURCE_FAMILIES}
    for row in rows:
        family = row["source_family"] if row.get("source_family") in SOURCE_FAMILIES else "broad_search"
        per_family_run[family]["discovered"] += 1
        if _verified(row): per_family_run[family]["verified"] += 1
        if _corroborated(row): per_family_run[family]["corroborated"] += 1
    for family, run in per_family_run.items():
        stat = memory.setdefault("families", {}).setdefault(family, {})
        for key in ("discovered", "verified", "corroborated"):
            stat[key] = _int(stat.get(key)) + _int(run.get(key))
        stat["last_seen"] = _now() if run["discovered"] else stat.get("last_seen")

    channel_failures = _parse_channel_failures(social)
    for row in channel_failures.values():
    ...
    # Bounded memory. Highest-risk learned cells are retained first.
    if len(memory.get("cells", {})) > MAX_MEMORY_CELLS:
        ranked_cells = sorted(
            memory["cells"].items(),
            key=lambda kv: (_int(kv[1].get("unverified_lead_streak")), _int(kv[1].get("candidate_runs"))),
            reverse=True,
        )
        memory["cells"] = dict(ranked_cells[:MAX_MEMORY_CELLS])

    memory["updated_at"] = _now()
    old = _load_memory(memory_path) if memory_path.exists() else None
    if old:
        atomic_write_json(memory_path.with_suffix(memory_path.suffix + ".bak"), old, suffix=".source-gap.bak.tmp")
    ...
    preference = preferred_families(memory)
    priorities = []
    for cell in audit["priority_cells"]:
        ...
        item["preferred_families"] = preference
        priorities.append(item)
    priorities.sort(key=lambda x: (x["risk_score"], x["unverified_lead_streak"], x["candidate_count"]), reverse=True)
    ...
    report = {
        ...
        "source_family_order": preference,
        "source_family_stats": family_rows,
        "channel_access_state": channel_failures,
...
class _ResultParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if href and ("result__a" in classes or "result-link" in classes):
            self.current_href = html.unescape(href)
            self.current_text = []
    ...
    def handle_endtag(self, tag):
        if tag.lower() == "a" and self.current_href:
            title = _short(" ".join(self.current_text), 220)
            if title:
                self.results.append((self.current_href, title))
            self.current_href = ""
            self.current_text = []
...
def _retry_after_seconds(headers) -> int | None:
    ...
    if raw is None:
        return None
    try:
        return max(1, min(86_400, int(str(raw).strip())))
    except (TypeError, ValueError, OverflowError):
        return None


def _secondary_sites(region: str) -> list[tuple[str, str]]:
    sites = [("community", "reddit.com")]
    if region == "KR":
        sites = [("secondary_wiki", "namu.wiki"), ("community", "blog.naver.com")] + sites
    return sites


def _search_secondary(game: str, region: str, topic: str, family: str, domain: str) -> tuple[list[dict], dict]:
    if family not in {"secondary_wiki", "community"} or domain not in SECONDARY_HOSTS:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 TCG-Grader-GapDiscovery/1.0"})
    try:
        with safe_urlopen(req, timeout=SEARCH_TIMEOUT, allowed_hosts=DDG_HOSTS) as response:
    ...
    parser.feed(raw)
    rows = []
    allowed_domain = domain.lower()
    for href, title in parser.results:
        target = _decode_ddg_target(href)
        host = _host(target)
        if not target.startswith("https://"):
            continue
        # Exact domain or subdomain only; search result links are stored, not fetched.
        if host != allowed_domain and not host.endswith("." + allowed_domain):
            continue
        expected_family = SECONDARY_HOSTS.get(host) or SECONDARY_HOSTS.get(allowed_domain)
        ...
        if len(rows) >= MAX_RESULTS_PER_QUERY:
            break
    return rows, {"ok": True, "query": query[:300], "result_count": len(rows), "family": family, "domain": domain}


def collect_secondary_leads(priority_cells: list[dict] | None, max_queries: int = MAX_SECONDARY_QUERIES) -> tuple[list[dict], dict]:
    """Search public indexes for high-risk gaps; never fetch the discovered page."""
    ...
    for cell in (priority_cells or [])[:MAX_PRIORITY_CELLS]:
        game = _game(cell.get("game")); region = _region(cell.get("region")); topic = str(cell.get("topic") or "")
        ...
        for family, domain in _secondary_sites(region):
            ...
            for row in found:
                ...
                if source and source not in seen_urls:
                    seen_urls.add(source); rows.append(row)
        if query_count >= limit or access_blocked or rate_limited:
            break
    return rows[:80], {
        "configured": True,
...
if __name__ == "__main__":
    try:
        social = json.loads(safe_read_text(ROOT / "social_event_candidates.json"))
    except Exception:
        social = {}
    try:
        supplementary = json.loads(safe_read_text(ROOT / "supplementary_candidates.json"))
    except Exception:
        supplementary = {}
    report = observe_pipeline(social, supplementary, [])
