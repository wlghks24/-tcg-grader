...
for label, old in (("init", old_init), ("sample", old_sample), ("finding", old_finding)):
    if text.count(old) != 1:
        raise SystemExit(f"{label} anchor mismatch: {text.count(old)}")
text = text.replace(old_init, new_init).replace(old_sample, new_sample).replace(old_finding, new_finding)
...
if __name__ == "__main__":
    unittest.main()
''', encoding="utf-8")
print("patched collection_verification_gate.py and wrote v248 regression")
