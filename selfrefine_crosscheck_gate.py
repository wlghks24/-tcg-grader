...
from shared_self_learning.contracts import assert_canonical_factual_type, assert_passive_exchange_payload
...
def _load_records(path: Path) -> list[dict[str, Any]]:
    ...
    if path.suffix.lower() == ".jsonl":
        ...
        for line_no, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            assert_canonical_factual_type(value.get("information_family"), label=f"{path}:{line_no}.information_family")
            rows.append(value)
        return rows
    ...
    for index, row in enumerate(value):
        assert_canonical_factual_type(row.get("information_family"), label=f"{path}:records[{index}].information_family")
    return value
