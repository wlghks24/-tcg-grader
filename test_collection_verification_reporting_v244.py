...
class CollectionVerificationReportingV244Tests(unittest.TestCase):
    def test_no_healthy_company_finding_excludes_other_company_degraded_samples(self):
        ...
        sample_sources = {row.get("source") for row in target.get("degraded_samples", [])}
        self.assertEqual(sample_sources, {"bgs-pricing", "bgs-news"})
        self.assertNotIn("psa-us-pricing", sample_sources)
