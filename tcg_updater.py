...
PUBLIC_STATIC_FILES={
    'index.html','icon.svg','manifest.webmanifest','sw.js','feature_category_nav.css','feature_category_nav.js','grading_vision_engine.js','grading_accuracy_v99.js','card_identity_recognition.js','manual_dual_photo_bridge.js',
...
def load_db():
    data=load_json_file(DB,default_db())
...
def load_json_file(path, fallback):
    cache_path=os.path.abspath(os.fspath(path))
    try:
    ...
    except (OSError,ValueError,TypeError):return copy.deepcopy(fallback)

def clear_json_file_cache():
    """Clear read-through JSON snapshots for tests and exceptional maintenance."""
    with JSON_FILE_CACHE_LOCK:
        JSON_FILE_CACHE.clear()
...
def _refresh_ai_auto_tracking(data, trigger):
    """Attach a public summary while detailed tracking state stays device-local."""
    try:
        import ai_auto_tracker
        report=ai_auto_tracker.run_once(trigger=f"tcg-updater:{trigger}",runtime_live=True)
        summary={
            'status':report.get('status'),'generated_at':report.get('generated_at'),
...
if __name__=='__main__':
    ...
    print('AI 통합 자동추적: 전체 업데이트 직후 + 매 1시간 기능·수집·시세추적·SELF-REFINE 상태 점검',flush=True)
