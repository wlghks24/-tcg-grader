...
def _persistent_variant(row: dict[str, Any], factual_type: str) -> str:
    if factual_type == "completed_sale":
        # Completed sales are only comparable when both domains preserve the same
        # underlying transaction lineage. This prevents cross-joining unrelated sales.
...
def _load_persisted_snapshot(
    ...
    *,
    ...
    if payload.get("namespace") != expected_namespace:
        raise ValueError(
            f"{path}: namespace mismatch; expected={expected_namespace!r} "
            f"actual={payload.get('namespace')!r}"
        )
    ...
    if failed_validation:
        raise ValueError(
            f"{path}: persisted validation flags are not PASS: {failed_validation}"
        )
    ...
    for index, row in enumerate(facts):
        ...
        factual_type = assert_canonical_factual_type(
            row.get("fact_type"),
            label=f"{path}:facts[{index}].fact_type",
        )
        ...
        if not canonical_key or not lineage_key:
            raise ValueError(
                f"{path}: facts[{index}] requires canonical_key and lineage_key"
            )
        ...
        if verification_status not in PERSISTED_VERIFICATION_STATES:
            raise ValueError(
                f"{path}: facts[{index}] invalid verification_status "
                f"{verification_status!r}"
            )
        if verification_status != "verified":
            # Cross-domain factual comparison is evidence, not a candidate merger.
            continue
        ...
        if not source_role or not source_locator or not observed_text:
            raise ValueError(
                f"{path}: verified facts[{index}] missing "
                "source_role/source_locator/observed_at"
            )

        observed = _aware_utc(
            observed_text,
            label=f"{path}:facts[{index}].observed_at",
        )
        ...
        mapped = {
            ...
            # Persisted exchange has no confidence field. Never invent 1.0/0.5.
            "confidence": 0.0,
            "lineage_key": lineage_key,
        }
        key = (
            mapped["information_family"],
        ...
        if prior is not None and prior["value"] != mapped["value"]:
            raise ValueError(
                f"{path}: local canonical conflict for {key}; "
                "a finalized snapshot cannot contain divergent verified values"
            )
        if prior is None:
            unique[key] = mapped
    ...
    if not main_meta.get("available") or not instagram_meta.get("available"):
        ...
        if report_output is not None:
            atomic_write_json(
                report_output,
                result,
                suffix=".crosscheck-persisted.tmp",
            )
        return result
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(dir=EXCHANGE) as tmp:
        ...
        persisted_main.write_text(
            json.dumps(
                _sample_persisted_snapshot("MARKET_ANALYSIS", lineage="main-release"),
                ensure_ascii=False,
            ),
            encoding="utf-8",
        )
        persisted_instagram.write_text(
            json.dumps(
                _sample_persisted_snapshot("IG_CARDINFO", lineage="ig-release"),
                ensure_ascii=False,
            ),
            encoding="utf-8",
        )
        ...
        assert persisted["source_mode"] == "persisted_tcg_crosscheck", persisted

        building = _sample_persisted_snapshot("IG_CARDINFO", lineage="ig")
        building["status"] = "building"
        building["finalized_at"] = None
        building["facts"] = []
        persisted_instagram.write_text(
        ...
        assert unavailable["unavailable_reasons"]["instagram_content"] == "not_finalized"
...
def main() -> int:
    ...
    parser.add_argument("--main-persisted", default=str(PERSISTED_MAIN))
    parser.add_argument("--instagram-persisted", default=str(PERSISTED_INSTAGRAM))
    parser.add_argument(
    ...
    if args.from_persisted or args.persisted:
        result = run_from_persisted(
            main_snapshot=Path(args.main_persisted),
            instagram_snapshot=Path(args.instagram_persisted),
        ...
        if not args.main_input or not args.instagram_input:
            raise SystemExit(
                "--main-input and --instagram-input are required unless "
                "--from-persisted is used"
            )
        result = run_bridge(
