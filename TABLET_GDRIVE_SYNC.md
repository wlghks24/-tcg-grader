...
Lenovo/Termux 태블릿이 12시간마다 자동으로 확인하여 해시·스키마·프로젝트 게이트를 통과한 경우에만
17개 공개 런타임 JSON을 원자적으로 교체합니다.

기본 확인 시간은 태블릿 현지시간 기준 **08:00 / 20:00**입니다. 재부팅 시에는 추가 동기화를 즉시 실행하지 않고 `crond`만 복구하여, 불필요하게 확인 빈도가 늘어나지 않게 합니다.
