...
1. `entry_file`: 현재 질의에 대한 **대표 시작 파일 1개**입니다.
2. `entry_files`: 호환성용 목록이며 알려진 기능에서는 항상 0개 또는 1개만 반환합니다.
3. `alternate_entry_files`: 같은 기능군에서 필요할 수 있는 대체 시작 후보입니다. 대표 시작점과 경쟁하지 않습니다.
4. `entrypoint_reason`: 기본 시작점인지(`group_default`) 질의 키워드로 승격된 것인지(`query_specific_rule`) 표시합니다.
