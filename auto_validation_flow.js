...
function syncIdentity(){
 ...
 const key=makeKey();
 if(key&&$('learningCardKey'))$('learningCardKey').value=key;
}
...
function syncManualPanel(){syncIdentity();syncOfficialLink()}
function mount(){
 if($('autoValidationPanel'))return true;
 const root=$('v30validation');if(!root)return false;
