...
function mount(){
 if($('gradingProxyPanel'))return true;
 const anchor=$('gradingCostPanel')||$('autoGradeMarketFlow')||$('gradingEconomics');
 if(!anchor)return false;
 const s=document.createElement('section');
 s.id='gradingProxyPanel';
 s.className='card grading-proxy-panel';
 ...
 s.querySelectorAll('.gpp-chip').forEach(b=>b.addEventListener('click',()=>{
   s.querySelectorAll('.gpp-chip').forEach(x=>x.classList.remove('active'));
   b.classList.add('active');
   render(window.__gradingProxyData||{},b.dataset.grader);
 }));
 $('gradingProxyRefresh')?.addEventListener('click',()=>load(true));
 load(false);
 return true;
}
...
async function load(force){
 const box=$('gradingProxyRows');if(!box)return;
 box.textContent='대행비용 확인 중…';
 try{
   const r=await fetch('/api/grading-proxy-costs?force='+(force?'1':'0')+'&t='+Date.now(),{cache:'no-store'});
   const j=await r.json();
   if(!r.ok||!j.ok)throw new Error(j.error||'load failed');
   window.__gradingProxyData=j;
   const active=document.querySelector('.gpp-chip.active')?.dataset.grader||'ALL';
   render(j,active);
 }catch(_){
   box.innerHTML='<span class="gpp-warn">태블릿/PC 서버 연결 후 대행비용을 확인할 수 있습니다.</span>';
 }
}
function boot(){
 let tries=0;
 ...
 mount();
 setInterval(()=>{if($('gradingProxyPanel'))load(false)},6*60*60*1000);
}
