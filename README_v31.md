...
- 수집 자가학습은 게임·출처·검색어별 성공률, 사진 확보율, 오류, 응답시간과 공식검증 성공을 누적하여 다음 검색어 순서를 자동 최적화하고, 미사용 검색어 탐색과 실패경로 복구기회도 계속 유지
- 공식검증된 카드 식별자만 후속 검색어로 학습하며, 판매글·미검증 사진은 검색 신뢰도나 등급 정답으로 승격하지 않음
- 라이브러리 등급사진은 하위 폴더까지 재귀 탐색하고, 1차 OCR에서 미해결된 경우에만 추가 크롭·판독을 실행하며, 동일 사진은 OCR을 재사용하고 미검증 인증번호는 공식조회 대기열로만 분리
