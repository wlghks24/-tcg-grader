...
function rarity(g,text){const s=' '+norm(text)+' ';const ordered=g==='pokemon'?['SAR','UR','HR','SR','AR','CHR','CSR','ACE SPEC','RRR','RR','R','U','C','PROMO']:g==='onepiece'?['MANGA','SP','SEC','L','SR','R','UC','C','P']:g==='naruto'?['MYTHIC','SECRET','SUPER RARE','RARE','UNCOMMON','COMMON','PROMO']:[];for(const r of ordered){const rx=new RegExp('(?:^|[^A-Z])'+r.replace(' ','\\s+')+'(?:$|[^A-Z])');if(rx.test(s))return r}return'UNKNOWN'}
