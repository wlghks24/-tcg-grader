...
function diagnosticHtml(payload){
 ...
 return `<div class="gpd-diagnostics"><div><b>사진 OCR</b><span>신규시도 ${n(probe.attempted)} · 신규정상 ${n(probe.validated)} · 보관검증 ${n(probe.prevalidated_library)} · 문자판독 ${n(probe.ocr_readable)} · 인증번호 ${n(probe.certs_extracted)}</span></div><div><b>공식 인증조회</b><span>저장검증 ${n(official.registry_matches)} · 실시간 ${n(official.live_verified)}/${n(official.live_attempts)} · 충돌 ${n(official.conflicts)}</span></div><div><b>수집 자가학습 v${n(learning.version)}</b><span>검색학습 ${n(learning.query_runs)}회 · 검색어 ${n(learning.queries_tracked)}개 · 업체경로 ${n(learning.grader_routes_tracked)}개 · 측정사진학습 ${n(learning.measurement_ready_feedback)}건 · 중복학습 차단 ${n(learning.duplicate_feedback_ignored)}건</span></div><div><b>연결 상태</b><span>Google CSE ${config.google_cse_configured?'연결':'미설정·공개검색 폴백'} · eBay API ${config.ebay_oauth_configured?'연결':'미설정·공개검색 폴백'}</span></div>${errors.length?`<div class="gpd-errors"><b>최근 폴백 원인</b><span>${errors.map(esc).join(' · ')}</span></div>`:''}</div>`;
}
function render(payload){
 ...
 <div class="gpd-section"><h3>등급사별 확보량</h3><div class="gpd-companies">${COMPANIES.map(c=>`<div class="gpd-company"><b>${c}</b><strong>${byCompany[c].toLocaleString()}</strong><span>장 · 사진검증 ${n(companyStats[c]?.validated_images)} · 측정참고 ${n(companyStats[c]?.measurement_ready)} · 공식 ${n(companyStats[c]?.verified_references)}</span></div>`).join('')}</div></div>
