...
function insertPanel(){
 ...
 <details class="gpd-manual"><summary>📷 자동등록 실패 사진 수동등록</summary><form id="gpdManualForm"><div class="gpd-manual-grid"><label>등급사<select id="gpdManualCompany" required>${COMPANIES.map(c=>`<option value="${c}">${c}</option>`).join('')}</select></label><label>카드게임<select id="gpdManualGame" required>${Object.entries(GAMES).map(([value,label])=>`<option value="${value}">${label}</option>`).join('')}</select></label><label>표시 등급<input id="gpdManualGrade" type="number" min="1" max="10" step="0.5" value="10" required></label><label>인증번호<input id="gpdManualCert" type="text" minlength="6" maxlength="24" autocomplete="off" required placeholder="슬랩 인증번호"></label><label>카드명(선택)<input id="gpdManualCardName" type="text" maxlength="180" placeholder="예: 피카츄"></label><label>카드번호(선택)<input id="gpdManualCardNumber" type="text" maxlength="60" placeholder="예: 001/100"></label></div><label class="gpd-photo-label">등급 슬랩 앞면 사진<input id="gpdManualPhoto" type="file" accept="image/jpeg,image/png" required></label><div class="gpd-manual-policy">JPG·PNG 1장만 등록됩니다. 서버에서 파일형식·해상도·중복을 검사하고 OCR과 공식 인증조회가 일치한 사진만 참고학습에 반영합니다. 수동등록 사진은 RAW 결함 보정학습에 사용하지 않습니다.</div><button id="gpdManualSubmit" type="submit">사진 안전등록</button><div id="gpdManualStatus" class="gpd-manual-status">PC·태블릿 로컬 서버에서 사용할 수 있습니다.</div></form><div id="gpdManualRows" class="gpd-manual-rows"></div></details>
...
function manualState(row){if(row.official_result===true)return ['공식검증 참고학습','ok'];if(row.verification_state==='deferred_by_cooldown')return ['등급사 쿨다운 대기','wait'];if(row.status==='quarantine')return ['불일치 격리','warn'];if(['queued','ocr_running'].includes(row.verification_state))return ['OCR·공식검증 중','wait'];if(row.verification_state==='processing_failed')return ['처리 재시도 필요','warn'];return ['공식검증 대기','wait']}
function renderManualRegistrations(payload){
 ...
 target.innerHTML=`<h4>최근 수동등록</h4>${rows.slice(0,10).map(row=>{const [label,klass]=manualState(row),retry=row.official_result!==true;return `<div class="gpd-manual-row"><div><b>${esc(row.company)} ${esc(row.claimed_grade)} · ${esc(GAMES[row.game]||row.game)}</b><span>인증 ${esc(row.certification_id)}${row.card_name?` · ${esc(row.card_name)}`:''}</span><small>${esc(row.image_width)}×${esc(row.image_height)} · ${esc(row.created_at)}</small></div><strong class="${klass}">${label}</strong>${retry?`<button type="button" data-retry="${esc(row.registration_id)}">재검증</button>`:''}</div>`}).join('')}`;
}
async function loadManualRegistrations(){
 const target=document.getElementById('gpdManualRows');try{const response=await fetch('/api/graded-photo-manual-registrations?_='+Date.now(),{cache:'no-store'});if(!response.ok)throw new Error('manual-status');renderManualRegistrations(await response.json())}catch(_){if(target)target.innerHTML=location.hostname.endsWith('github.io')?'<div class="gpd-empty">수동등록은 PC·태블릿의 로컬/Tailscale 서버 주소에서 실행하세요.</div>':'<div class="gpd-error">수동등록 현황을 불러오지 못했습니다.</div>'}
}
async function submitManualRegistration(event){
 ...
 try{const image=await normalizedPhoto(file);status.textContent='안전등록 후 OCR·공식 인증조회 대기열에 추가 중…';const body={company:document.getElementById('gpdManualCompany').value,game:document.getElementById('gpdManualGame').value,grade:Number(document.getElementById('gpdManualGrade').value),certification_id:document.getElementById('gpdManualCert').value,card_name:document.getElementById('gpdManualCardName').value,card_number:document.getElementById('gpdManualCardNumber').value,filename:file?.name||'',image_data_url:image};const response=await fetch('/api/graded-photo-manual-registration',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body),cache:'no-store'});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||`등록 실패(${response.status})`);status.textContent=data.duplicate?'같은 사진이 이미 등록되어 기존 항목을 표시합니다.':'등록 완료 · OCR과 공식 인증조회를 진행합니다.';document.getElementById('gpdManualPhoto').value='';for(let i=0;i<12;i++){await sleep(1500);await loadManualRegistrations()}await load();
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>load(),{once:true});else load();setInterval(()=>{if(!running&&!manualSubmitting)load()},60000);
})();
