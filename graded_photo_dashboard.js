...
function isVerified(r){const s=statusOf(r);return r.verified===true||r.official_verified===true||s.includes('verified')||s.includes('공식검증')}
...
function latestOf(rows,payload){
  const vals=rows.map(r=>r.collected_at||r.updated_at||r.verified_at||r.created_at).filter(Boolean).sort();
  return vals.at(-1)||payload.updated_at||payload.collected_at||payload.generated_at||'-';
}
function insertPanel(){
  ...
  box.querySelector('#gpdRefresh')?.addEventListener('click',()=>load(true));
  return box;
}
function render(payload){
  ...
  body.innerHTML=`
    <div class="gpd-summary">
      <div><span>전체 후보</span><b>${total.toLocaleString()}건</b></div>
      <div><span>공식검증</span><b>${verified.toLocaleString()}건</b></div>
      <div><span>격리 후보</span><b>${quarantine.toLocaleString()}건</b></div>
      <div><span>보정학습 가능</span><b>${eligible.toLocaleString()}건</b></div>
    </div>
...
async function load(force=false){
  insertPanel(); const body=document.getElementById('gpdBody'); if(body&&force) body.innerHTML='<div class="gpd-loading">다시 확인하는 중…</div>';
  try{
    const res=await fetch('/api/graded-photo-learning?_='+(force?Date.now():'1'),{cache:'no-store'});
    if(!res.ok) throw new Error('HTTP '+res.status);
    render(await res.json());
  }catch(e){if(body) body.innerHTML='<div class="gpd-error">현황을 불러오지 못했습니다. 서버가 켜져 있는지 확인하세요.</div>'}
}
