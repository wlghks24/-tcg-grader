...
def _normalize_fact(row: dict[str, Any]) -> dict[str, Any] | None:
    ...
    fact_type = FACT_TYPE_ALIASES.get(raw_type, raw_type)
    ...
    fact = {
        ...
        "source_role": _clean(row.get("source_role") or row.get("source_code") or "main_verified"),
...
def export_snapshot(
    ...
    if reread.get("namespace") != "MARKET_ANALYSIS" or reread.get("facts") != payload.get("facts"):
