...
This module is deliberately network-free. It protects the LAN/Tailscale HTTP
service from public-source clients and prevents accidental request floods against
PSA/BGS/CGC/TAG/BRG official lookup pages.

Important: this guard does not bypass a grading company's 403/429 response. Its
job is to avoid creating unnecessary local 429 responses, respect provider
cooldowns, and keep Tailscale clients working normally.
"""
...
# Local pacing is intentionally light. The previous 60-second/2-per-3-min rule
# caused many 429 responses generated by our own server even when the official
# provider had not rate-limited the user.
...
_IPV4_LAN_NETWORKS = (
    ...
    # Tailscale uses CGNAT space 100.64.0.0/10 for tailnet addresses.
...
def client_network_allowed(value: Any) -> bool:
    """Allow only loopback, LAN, or Tailscale/CGNAT source addresses."""
...
def _default_cooldown(status: int) -> float:
    """Conservative retry delay when the provider does not send Retry-After."""
    if status == 429:
        return 5 * 60.0
...
class OfficialLookupGuard:
    """Thread-safe lightweight pacing plus provider-directed cooldown handling."""
    ...
    def record_result(self, company: Any, result: Any, *, now: float | None = None) -> dict[str, Any]:
        ...
        with self._lock:
            if status in BLOCK_HTTP_STATUSES or payload.get("blocked_or_challenged") is True:
                ...
                # Repeated provider blocks back off progressively; no proxy/IP rotation
                # or other access-control bypass is attempted.
