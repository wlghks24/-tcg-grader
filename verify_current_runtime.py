...
def _commands():
    ...
    rows=[
      ...
      ("card_core_static_regressions",[py,"-m","unittest","-v",
       ...
       "test_card_identity_ambiguity_v320.py","test_card_variant_market_precision_v321.py","test_multi_market_price_collector.py","test_tablet_runtime_manifest_ui_assets_v254.py"],360,False),
    ...
    if shutil.which("node"):
        rows += [("card_core_node_regressions",[py,"-m","unittest","-v",
                  ...
                  "test_card_identity_market_precision_v309.py","test_card_region_conflict_failclosed_v318.py",
                  "test_card_identity_ambiguity_v320.py","test_card_variant_market_precision_v321.py"],360,False),
    ...
    return rows
def run(passes:int):
    passes=max(1,min(5,int(passes))); payload={"schema_version":1,"engine":"current-main-v321-card-variant-market-failclosed","started_at":_now(),"passes":[],"ok":False}
