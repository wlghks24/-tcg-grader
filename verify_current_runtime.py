...
def _commands():
    ...
    rows=[
      ...
      ("card_core_static_regressions",[py,"-m","unittest","-v",
       ...
       "test_card_identity_market_precision_v309.py","test_card_region_conflict_failclosed_v318.py","test_card_region_label_alias_v319.py",
    ...
    if shutil.which("node"):
        rows += [("card_core_node_regressions",[py,"-m","unittest","-v",
                  ...
                  "test_card_identity_market_precision_v309.py","test_card_region_conflict_failclosed_v318.py","test_card_region_label_alias_v319.py"],360,False),
    ...
    return rows
def run(passes:int):
    passes=max(1,min(5,int(passes))); payload={"schema_version":1,"engine":"current-main-v318-card-region-conflict-failclosed","started_at":_now(),"passes":[],"ok":False}
