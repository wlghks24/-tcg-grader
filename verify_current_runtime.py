...
def _commands():
    ...
    if shutil.which("node"):
        rows += [("card_core_node_regressions",[py,"-m","unittest","-v",
                 ...
                 ("tablet_ui_repair",["node","verify_tablet_ui_repair.js"],60,False),
    ...
    return rows
def run(passes:int):
    passes=max(1,min(5,int(passes))); payload={"schema_version":1,"engine":"current-main-v329-market-generation-failclosed-fx-ui","started_at":_now(),"passes":[],"ok":False}
