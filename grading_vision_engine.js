(function(root,factory){
  ...
  function analyzeSurface(baseInput,obliqueInput=null,baseBounds=null,providedConfig={},providedQuality=null,providedObliqueQuality=null,providedObliqueBounds=null){
    const baseQuality=providedQuality||analyzeQuality(baseInput),baseOuter=baseBounds||detectOuterBounds(baseInput),base=detectScratchCandidates(baseInput,baseOuter,providedConfig);let oblique=null,obliqueQuality=null;
    if(obliqueInput){obliqueQuality=providedObliqueQuality||analyzeQuality(obliqueInput);oblique=detectScratchCandidates(obliqueInput,providedObliqueBounds||detectOuterBounds(obliqueInput),providedConfig)}
  ...
  function analyzeFourQuadrants(baseInput,obliqueInput=null,providedBounds=null,providedConfig={},providedSurface=null,providedQuality=null,providedObliqueBounds=null){
    const profile=gameProfile(providedConfig.game),config={...DEFAULT_CONFIG,...providedConfig},bounds=providedBounds||detectOuterBounds(baseInput),quality=providedQuality||analyzeQuality(baseInput),surface=providedSurface||analyzeSurface(baseInput,obliqueInput,bounds,config,quality,null,providedObliqueBounds),precisionConfig={...config,minLineRatio:Math.max(.10,config.minLineRatio*.50),houghVoteThreshold:Math.max(18,config.houghVoteThreshold),scratchContrastMin:Math.max(30,config.scratchContrastMin)},precisionBase=detectScratchCandidates(baseInput,bounds,precisionConfig),precisionOblique=obliqueInput?detectScratchCandidates(obliqueInput,providedObliqueBounds||detectOuterBounds(obliqueInput),precisionConfig):null,ids=['tl','tr','bl','br'],rows={};
    for(const id of ids){
  ...
  function analyzeEightZones(baseInput,obliqueInput=null,providedBounds=null,providedConfig={},providedQuality=null,providedObliqueBounds=null){
    const profile=gameProfile(providedConfig.game),config={...DEFAULT_CONFIG,...providedConfig},bounds=providedBounds||detectOuterBounds(baseInput),quality=providedQuality||analyzeQuality(baseInput),precisionConfig={...config,minLineRatio:Math.max(.065,config.minLineRatio*.34),houghVoteThreshold:Math.max(16,config.houghVoteThreshold-1),scratchContrastMin:Math.max(30,config.scratchContrastMin)},base=detectScratchCandidates(baseInput,bounds,precisionConfig),oblique=obliqueInput?detectScratchCandidates(obliqueInput,providedObliqueBounds||detectOuterBounds(obliqueInput),precisionConfig):null,ids=['r1l','r1r','r2l','r2r','r3l','r3r','r4l','r4r'],zones={};
    for(const id of ids){
  ...
  function analyzeGradingHierarchy(baseInput,obliqueInput=null,providedBounds=null,providedConfig={}){
    const bounds=providedBounds||detectOuterBounds(baseInput),quality=analyzeQuality(baseInput),obliqueQuality=obliqueInput?analyzeQuality(obliqueInput):null,obliqueBounds=obliqueInput?detectOuterBounds(obliqueInput):null,centering=measureCentering(baseInput,bounds,quality),surface=analyzeSurface(baseInput,obliqueInput,bounds,providedConfig,quality,obliqueQuality,obliqueBounds),whitening=analyzeWhitening(baseInput,bounds),quadrants=analyzeFourQuadrants(baseInput,obliqueInput,bounds,providedConfig,surface,quality,obliqueBounds),zones=analyzeEightZones(baseInput,obliqueInput,bounds,providedConfig,quality,obliqueBounds),surfaceRisk=Math.max(surface.risk,quadrants.surfaceWorstRisk,zones.surfaceWorstRisk),edgeRisk=Math.max(whitening.edgeRisk,quadrants.edgeWorstRisk,zones.edgeWorstRisk),cornerRisk=Math.max(whitening.cornerRisk,quadrants.cornerWorstRisk,zones.cornerWorstRisk),defectRisk=Math.round(clamp(Math.max(surfaceRisk,edgeRisk*.90,cornerRisk*.95),0,100)),confidence=Math.round(clamp(Math.min(quality.score,centering.confidence||0,surface.confidence,quadrants.confidence,zones.confidence),0,99));
