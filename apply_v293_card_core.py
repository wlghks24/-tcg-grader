...
# Reapply only the verified functional v292 tree onto the latest main.  Do not
# copy v292 data snapshots or one-shot workflows: latest main data remains SSOT.
for name in PORT_FILES:
    (ROOT / name).write_text(show(V292_REF, name), encoding="utf-8")

# v293: real 7/30/90-day market filtering helper (unit-testable in Node/browser).
...
# Wire period helper before the inline v13 price search.
index = ROOT / "index.html"
replace_exact(
    index,
    '<script src="./grading_probability_v292.js?v=292"></script>',
    '<script src="./grading_probability_v292.js?v=292"></script>\n<script src="./market_period_filter_v293.js?v=293"></script>',
    count=1,
)
replace_exact(
    index,
...
# Asking/listing/reference values are not all completed transactions.
text = index.read_text(encoding="utf-8")
text = text.replace('현재 거래시세<b>', '현재 시장가격<b>')
index.write_text(text, encoding="utf-8")

# v293 generation fail-closed: reject implausible far-future OCR years and accept
# common region aliases used by tablet/manual workflows.
...
replace_exact(
    ...
    " if(['JP','JPN','JA','JAPAN','JAPANESE','日本','日本版','日版','JP版'].includes(r))return 'JP';\n if(['US','USA','EN','ENG','ENGLISH'].includes(r))return 'US';\n if(['KR','KOR','KO','KOREA','KOREAN','한국','한국판'].includes(r))return 'KR';",
    count=1,
)

# Make the non-calibrated semantics explicit in the cockpit title.
...
# Add the new helper to all executable/static tablet contracts.
...
# Preserve latest main price rows, adding only structured provenance to the two
# existing non-zero exact-grade seed observations used by valuation.
...
# Additional v293 regression coverage.
...
class CardCoreCrosscheckV293Tests(unittest.TestCase):
    ...
    def test_market_period_filter_enforces_7_30_90_day_windows(self):
        script = r"""
const p=require('./market_period_filter_v293.js');
const now=Date.parse('2026-09-23T00:00:00Z');
const out={
 d7inside:p.withinPeriod('2026-09-16','최근 7일',now),
 d7outside:p.withinPeriod('2026-09-15','최근 7일',now),
 d30inside:p.withinPeriod('2026-08-24','최근 30일',now),
 d30outside:p.withinPeriod('2026-08-23','최근 30일',now),
 d90inside:p.withinPeriod('2026-06-25','최근 90일',now),
 missing:p.withinPeriod('', '최근 7일', now),
 invalid:p.withinPeriod('not-a-date','최근 30일',now),
 future:p.withinPeriod('2026-09-25','최근 90일',now),
 days:[p.periodDays('최근 7일'),p.periodDays('최근 30일'),p.periodDays('최근 90일')]
};
process.stdout.write(JSON.stringify(out));
"""
        out = self.node_json(script)
        self.assertEqual([7, 30, 90], out["days"])
        self.assertTrue(out["d7inside"])
        self.assertFalse(out["d7outside"])
        self.assertTrue(out["d30inside"])
        self.assertFalse(out["d30outside"])
        self.assertTrue(out["d90inside"])
        self.assertFalse(out["missing"])
        self.assertFalse(out["invalid"])
        self.assertFalse(out["future"])

    def test_price_ui_uses_period_filter_and_market_price_semantics(self):
        page = (ROOT / "index.html").read_text(encoding="utf-8")
        self.assertIn('market_period_filter_v293.js?v=293', page)
        self.assertIn('TCGMarketPeriodV293.withinPeriod(observedAt,period,Date.now())', page)
        self.assertIn('현재 시장가격<b>', page)
        self.assertNotIn('현재 거래시세<b>', page)
...
const out={
 jpn:api.infer({game:'pokemon',region:'JPN',ocr_text:'©2019 Pokémon'}),
 kor:api.infer({game:'pokemon',region:'KOR',ocr_text:'©2020 Pokémon'}),
 eng:api.infer({game:'pokemon',region:'ENG',ocr_text:'©2020 Pokémon'}),
 future:api.infer({game:'pokemon',region:'JP',ocr_text:'©2099 Pokémon'})
};
process.stdout.write(JSON.stringify(out));
"""
        out = self.node_json(script)
        self.assertEqual(8, out["jpn"]["generation"])
        self.assertEqual(8, out["kor"]["generation"])
        self.assertEqual(8, out["eng"]["generation"])
        self.assertEqual("unknown", out["future"]["status"])
        self.assertIsNone(out["future"]["generation"])
    ...
    def test_probability_ui_is_explicitly_non_calibrated(self):
        ...
        self.assertIn('calibrated:false', page)
        self.assertIn('PSA 8/9/10 휴리스틱 분포', shell)
        self.assertIn("'probability_claim':False", server)
    ...
    def test_v293_assets_are_fail_closed_tablet_runtime_dependencies(self):
        ...
        self.assertIn("market_period_filter_v293.js", tablet_runtime_manifest.ACTIVE_RUNTIME_FILES)
        self.assertIn("market_period_filter_v293.js", tcg_updater.PUBLIC_STATIC_FILES)
        worker = (ROOT / "sw.js").read_text(encoding="utf-8")
        self.assertIn("const CACHE='tcg-v293-card-core-runtime';", worker)
        self.assertIn("'./market_period_filter_v293.js'", worker)
