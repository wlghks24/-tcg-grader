...
ENGINE = "v160-retry-reason-explainer"
MAX_DETAILS = 8
...
def _label_reason_codes(row: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    if probe_status == "retryable_failed" or row.get("image_revalidation_retryable") is True:
        if "429" in network_text or "rate" in network_text:
            codes.append(("rate_limited", "사이트 429/요청제한"))
        elif "403" in network_text or "blocked" in network_text or "challenge" in network_text:
            codes.append(("access_blocked", "사이트 403/접근제한"))
        elif "404" in network_text:
            codes.append(("image_404_retry", "사진주소 404·대체사진 재확인"))
        elif "timeout" in network_text or "timedout" in network_text:
            codes.append(("network_timeout", "네트워크 시간초과"))
        else:
...
def _is_retryable_candidate(row: dict[str, Any]) -> bool:
    if row.get("official_result") is True and not row.get("evidence_conflicts"):
        return False
    if row.get("image_revalidation_retryable") is True:
        return True
    if str(row.get("image_probe_status") or "").lower() == "retryable_failed":
        return True
    # Remaining quarantine rows are preserved until the two-pass delete gate has
    # enough evidence. Showing the reason is useful even when the retained-grace
    # branch, rather than a network error, kept the row.
    return str(row.get("status") or "").lower() in {"quarantine_candidate", "quarantine"}
...
def summarize_rows(rows: list[dict[str, Any]], *, max_details: int = MAX_DETAILS) -> dict[str, Any]:
    ...
    return {
        "engine": ENGINE,
        "retryable_count": len(details) if len(details) < max_details else sum(1 for row in rows if isinstance(row, dict) and _is_retryable_candidate(row)),
