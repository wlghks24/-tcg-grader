const CACHE='tcg-v31-market-watch-ko-31';
const CORE=['./','./index.html','./manifest.webmanifest','./releases.json','./promo_events.json','./market_prices.json','./market_watch.json','./exchange_rates.json','./icon.svg'];
self.addEventListener('install',event=>event.waitUntil(caches.open(CACHE).then(cache=>cache.addAll(CORE)).then(()=>self.skipWaiting())));
self.addEventListener('activate',event=>event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(key=>key!==CACHE).map(key=>caches.delete(key)))).then(()=>self.clients.claim())));
self.addEventListener('fetch',event=>{
  if(event.request.method!=='GET' || new URL(event.request.url).pathname.startsWith('/api/')) return;
  const url=new URL(event.request.url),fresh=event.request.mode==='navigate'||/\/(?:index\.html|releases\.json|promo_events\.json|market_prices\.json|market_watch\.json|exchange_rates\.json)$/.test(url.pathname);
