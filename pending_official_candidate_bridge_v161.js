...
async function normalize(file){
  ...
  const decoded=await decodedPhoto(file);
  try{
    ...
    const ctx=canvas.getContext('2d');if(!ctx)throw new Error('사진 변환 실패');
...
function officialOpenUrl(url,cert,company){const c=String(company||'').toUpperCase();if(c!=='BGS')return url;try{const u=new URL(url,location.href);u.searchParams.set('flag','1');u.searchParams.set('item_id',cert);u.searchParams.set('item_type','BGS');return u.toString()}catch(_){return url}}
...
async function render(){
  ...
  try{
    ...
    box.querySelectorAll('.gpd-pending-row').forEach(row=>{
      ...
      button?.addEventListener('click',async e=>{e.preventDefault();e.stopPropagation();const file=currentProof(row,input);if(!file){setState(state,'② 확인화면을 먼저 선택하세요.','error');return}button.disabled=true;reject.disabled=true;setState(state,'③ 등록 요청 접수 · 공식 조회 화면 OCR 일치검사 중…','waiting');try{const proof_image=await normalize(file);const data=await postProof({action:'complete_manual_verification',manual_verification_confirmed:true,candidate_id:cid,proof_image});if(!data.accepted){setState(state,'미등록: '+String(data.error||'정보 불일치'),'error');button.disabled=false;reject.disabled=false;return}if(data.verification_complete!==true||data.policy?.official_result!==true)throw new Error('최종 레지스트리 검증 미완료 · 학습자료로 승격하지 않았습니다.');proofFiles.delete(cid);setState(state,'✓ 공식검증 완료 · 학습자료 승격');setTimeout(()=>location.reload(),900)}catch(e){setState(state,'오류: '+String(e?.message||e),'error');button.disabled=false;reject.disabled=false}});
