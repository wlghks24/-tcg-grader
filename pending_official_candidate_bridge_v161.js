...
async function render(){
  ...
  try{
    ...
    box.querySelectorAll('.gpd-pending-row').forEach(row=>{
      ...
      input?.addEventListener('change',()=>{const file=input.files?.[0];const ready=!!file;button.disabled=!ready;reject.disabled=!ready;if(ready){pick.textContent='② 갤러리에서 확인화면 다시선택';setState(state,`✓ 확인화면 선택 완료: ${file.name} · 이제 ③ 또는 ④를 누르세요.`)}else{setState(state,'사진이 선택되지 않았습니다. ② 갤러리에서 확인화면 선택을 다시 누르세요.','waiting')}});
