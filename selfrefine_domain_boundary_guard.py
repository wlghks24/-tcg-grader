...
# Only passive, non-executable data may cross the domain boundary.
...
def main() -> int:
    ...
    # 1) Exchange area is data-only. Executable/serialized-code formats are forbidden.
    ...
    # 2) Python code may not import across Main <-> Instagram domains.
    for path in ROOT.rglob('*.py'):
        ...
        if rel.startswith(CONTENT_PREFIX):
            ...
            # Do not load/execute Main source by path either.
            ...
            if 'instagram_tcg_content/' in text and not rel.startswith(EXCHANGE_PREFIX):
                # Main may know the exchange contract, but must not address content source files.
                if '.py' in text or 'import' in text:
                    errors.append(f'{rel}: Main references Instagram source code path')

        # Cross-check consumers/producers may parse data, never execute learned/exchanged text.
    ...
    if errors:
        for error in errors:
            print(error)
        return 1
    print('SELFREFINE domain boundary: PASS (data cross-check allowed; code coupling blocked)')
    return 0
